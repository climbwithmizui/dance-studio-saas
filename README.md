...
- Dance Studioは月額500円（税込）。EvoLinkのアカウントと生成クレジットは利用者が別途用意します。
...
- `/guide`: Dance Studio使い方ガイド（概要は公開、技術的詳細は有効なサブスク契約者だけに表示）
- `/guide/evolink-api-key`: APIキー取得ガイド（ログイン＋有効なサブスクが必要）
- `/terms`: 利用規約（公開）
- `/privacy`: プライバシーポリシー（公開）
- `/tokushoho`: 特定商取引法に基づく表記（公開）

未契約者向けガイドでは、生成の流れ・2つの生成方法・料金・FAQだけを案内します。APIキー取得手順、対応形式・上限、技術的な注意点、トラブル対応は有効なサブスク契約者だけに表示します。法的ページは購入前の確認に必要なため全文公開します。
